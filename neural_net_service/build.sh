...
echo "Building neural_net_service version ${PROJECT_VERSION} with common version ${COMMON_VERSION}"
...
docker build \
    ...
    -t neural_net_service:${PROJECT_VERSION} \
    -f Dockerfile \
    .. # Build context is now the root directory
echo "docker build completed"
cd neural_net_service # Go back to the original directory (optional, but good practice)
