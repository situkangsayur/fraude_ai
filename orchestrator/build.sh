...
echo "Building orchestrator version ${PROJECT_VERSION} with common version ${COMMON_VERSION}"
...
docker build \
    ...
    -t orchestrator:${PROJECT_VERSION} \
    -f Dockerfile \
    .. # Build context is now the root directory
echo "docker build completed"
cd orchestrator # Go back to the original directory (optional, but good practice)